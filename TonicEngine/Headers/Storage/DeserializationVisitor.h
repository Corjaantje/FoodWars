...
public:
    ...
    void visit(const std::string &name,
               SerializationReceiver *receiver) override = 0; //todo: maybe add function with unique pointer

    virtual void visit(const std::string &name, std::unique_ptr<SerializationReceiver> &receiver) = 0;

    void visit(const std::string &name, std::vector<SerializationReceiver *> &receivers) override = 0;

    virtual void visit(const std::string &name, std::vector<std::unique_ptr<SerializationReceiver>> &receivers) = 0;
