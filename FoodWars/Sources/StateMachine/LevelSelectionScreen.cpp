...
LevelSelectionScreen::LevelSelectionScreen(std::shared_ptr<ScreenStateManager> context,
                                           std::shared_ptr<LevelLoader> levelManager) : IScreen(context),
                                                                                        _levelManager(levelManager),
                                                                                        _currentIndex(0),
                                                                                        mouseEventObservable(
                                                                                                _inputFacade->getMouseEventObservable().get()) {
    audioFacade = context->getFacade<AudioFacade>();
    _inputFacade->getKeyEventObservable()->IObservable<KeyEvent>::registerObserver(this);
    _renderList._shapes[0].push_back(createShape<ShapeSprite>(1600, 900, 0, 0, "ScreenLevelSelection.png"));

    // MainMenu
    SpriteButton *quitButton = new SpriteButton{*_inputFacade->getMouseEventObservable(), "",
                                                [c = _context]() { c->setActiveScreen<MainMenuScreen>(); }, 120, 120,
                                                10, 10, Colour{0, 0, 0, 0}};
    quitButton->addToRender(&_renderList);
    _sprites.push_back(quitButton);

    // Highscore
    //TODO ADD HIGHSCORE SCREEN BUTTON
    SpriteButton *highscorebutton = new SpriteButton{*_inputFacade->getMouseEventObservable(), "",
                                                     [c = _context]() { c->setActiveScreen<MainMenuScreen>(); }, 120,
                                                     120, 150, 10, Colour{0, 0, 0, 0}};
    highscorebutton->addToRender(&_renderList);
    _sprites.push_back(highscorebutton);

    generateLevelButtons();

    SpriteButton *previousButton = new SpriteButton{*_inputFacade->getMouseEventObservable(), "",
                                                    [this]() { swapLevels(false); }, 60, 60, 535, 444,
                                                    Colour{0, 0, 0, 0}};
    previousButton->addToRender(&_renderList);
    _sprites.push_back(previousButton);

    SpriteButton *nextButton = new SpriteButton{*_inputFacade->getMouseEventObservable(), "",
                                                [this]() { swapLevels(true); }, 60, 60, 1000, 444, Colour{0, 0, 0, 0}};
    ...
    for (int i = 0; i < _levels.size(); i++) {
        ...
        TextButton *button = new TextButton{*_inputFacade->getMouseEventObservable(),
                                            ...
                                            [c = _context, this, fileNum]() {
                                                ...
                                                _levelManager->loadLevel(fileNum, *gameLevel);
