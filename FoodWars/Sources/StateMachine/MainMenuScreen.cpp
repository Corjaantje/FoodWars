...
MainMenuScreen::MainMenuScreen(std::shared_ptr<ScreenStateManager> context) : IScreen(context) {
    ...
    _renderList._shapes[1].push_back(new ShapeSprite{1600, 900, 0, 0, "wallpaper2.png", 0});
    ...
    // Level Editor
    TextButton* levelEditorButton = new TextButton {*_inputFacade->getMouseEventObservable(),"Level Editor", [c = _context]() {  c->setActiveScreen<LevelEditorScreen>(); }, 370, 110, 615, 420, Colour{255,255,255,0}, Colour{255,255,255,0}, 0};
    levelEditorButton->addToRender(&_renderList);
    _buttons.push_back(levelEditorButton);

    // Upgrades
    TextButton* upgradesButton = new TextButton {*_inputFacade->getMouseEventObservable(),"Upgrades", [c = _context]() {  c->setActiveScreen<UpgradesScreen>(); }, 370, 110, 615, 540, Colour{255,255,255,0}, Colour{255,255,255,0}, 0};
    upgradesButton->addToRender(&_renderList);
    _buttons.push_back(upgradesButton);

    // Settings
    SpriteButton* settingsButton = new SpriteButton {*_inputFacade->getMouseEventObservable(), "settings.png", [c = _context]() {  c->setActiveScreen<SettingsScreen>(); }, 120, 120, 1335, 10, Colour{0,0,0,0}, 0};
    settingsButton->addToRender(&_renderList);
    _buttons.push_back(settingsButton);

    // Help
    TextButton* helpButton = new TextButton {*_inputFacade->getMouseEventObservable(),"Help", [c = _context]() {  c->setActiveScreen<GameScreen>(); }, 190, 100, 10, 680, Colour{255,255,255,0}, Colour{255,255,255,0}, 0};
    helpButton->addToRender(&_renderList);
    _buttons.push_back(helpButton);

    // Credits
    TextButton* creditsButton = new TextButton {*_inputFacade->getMouseEventObservable(),"Credits", [c = _context]() {  c->setActiveScreen<CreditScreen>(); }, 190, 100, 10, 790, Colour{255,255,255,0}, Colour{255,255,255,0}, 0};
